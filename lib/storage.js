 ...
 *  CreateDocumentB
 */
exports.createDocB = function(cookies, sName, base64binary, cabGuid, oAttr, fn) {
	...
	for (name in oAttr) {
		...
		attrList += '<string>' + name + '</string>';
		valList += '<string>' + oAttr[name] + '</string>';
		if (c == attrList.length - 1) {
			attrList += '<string>accessList</string>';
			valList += '<string><accessList>*standard*</accessList></string>';
		}
	};
	
	// define variables specific to the API call
	var soapAction = 'CreateDocumentB'
		, soapRequest = '<name>' + sName + '</name>' +
										'<cabGuid>' + cabGuid + '</cabGuid>' +
										'<docContent>' + base64binary + '</docContent>' +
										'<attrList>' + attrList + '</attrList>' +
										'<valueList>' + valList + '</valueList>';
